export async function placeComponentsEfficiently(viewType = "PCB") {
    ...
    // 创建进度条文本对象 (使用宋体!!)
    const lookupProgressText = await eda.sch_PrimitiveText.create(10, -50, '查找器件：等待开始...', 0, CONFIG.infoFontColor, CONFIG.progressFont, 16, false, false, false, 0);
    const placeProgressText = await eda.sch_PrimitiveText.create(10, -70, '放置器件：等待开始...', 0, CONFIG.infoFontColor, CONFIG.progressFont, 16, false, false, false, 0);
    ...
    async function placeBatch(list, startX, startY, dirX, dirY, type, progressObj, totalItems) {
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            if (type === "component") {
                ...
                const p = eda.sch_PrimitiveText.create(x, y, content, 0, CONFIG.errorFontColor, CONFIG.defaultFont, CONFIG.fontSize, false, false, false, 0);
                promises.push(p);
            }
    ...
    async function showFinalStatistics(total, success, fail) {
        ...
        const hitRate = (stats.queryCount + stats.cacheHitCount) > 0 
            ? ((stats.cacheHitCount / (stats.queryCount + stats.cacheHitCount)) * 100).toFixed(2) 
            : "0.00";
...
`.trim();
    ...
    async function createInfoTexts() {
        ...
        await eda.sch_PrimitiveText.create(10, -10, '放置过程中不要做其他操作', 0, CONFIG.uuidFontColor, CONFIG.defaultFont, 24, true, false, false, 0);
    }
