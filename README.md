# 当前插件暂时下架
由于不同版本 EDA 的 BUG 各不相同，当前插件只能在特定版本的 EDA 下基于 BUG 运行，BUG 一旦被修复，当前插件使用起来会出现异常。在想办法自动识别特定 API 的 BUG，基于 BUG 动态调整参数输入，使其能够正确处理  
